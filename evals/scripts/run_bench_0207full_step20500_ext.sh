...
echo "$(date +%H:%M) ext: waiting for step20500 aime25 to finish (BENCH_0207FULL_S20500_DONE)..."
while [ ! -f traces/BENCH_0207FULL_S20500_DONE ]; do sleep 60; done
# only proceed if aime25 went well (tool run produced a full set)
if [ "$(ls traces/bench_0207full_step20500_aime25_tool/*.jsonl 2>/dev/null|wc -l)" -lt 28 ]; then
...
for bench in aime26 hmmt_feb2025; do
  echo "===== S20500 NOTOOL $bench $(date +%H:%M) ====="
  python3 run_eval.py --task boxed_notool --input bench_${bench}.jsonl --output_dir traces/bench_0207full_step20500_${bench}_notool --max_parallel 48 $COMMON
done
for bench in aime26 hmmt_feb2025; do
  echo "===== S20500 TOOL $bench $(date +%H:%M) ====="
  python3 run_eval.py --task boxed_tool --input bench_${bench}.jsonl --output_dir traces/bench_0207full_step20500_${bench}_tool --max_parallel 24 --max_turns 64 --python_timeout 60 --max_output_characters 600 $COMMON
done
echo "===== S20500 EXT DONE $(date +%H:%M) ====="; touch traces/BENCH_0207FULL_S20500_EXT_DONE
