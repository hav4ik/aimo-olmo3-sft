"""
LOCAL TEST PATH — a 1B sibling of `Olmo-3-7B-SFT-local.py`, for exercising the
WHOLE Olmo SFT pipeline (HF->distcp convert -> train -> distcp checkpoint ->
convert -> upload) on a single small GPU. The target is the real published
`allenai/OLMo-2-0425-1B-Instruct` (an Olmo-2 1B; ~1B params, fits on a 24 GB
card). This is for PIPELINE testing, NOT recipe fidelity — performance and
hyperparameter correctness are explicitly out of scope. run.sh wires it up
under MODEL_SIZE=1b.

WHAT DIFFERS FROM THE 7B LOCAL SCRIPT (the ONLY substantive changes; everything
else — every Beaker stub and env-gated knob below — is byte-for-byte the 7B
local script, so the SAME env contract / run.sh machinery applies):
  A. MODEL FACTORY: `TransformerConfig.olmo2_1B_v2(vocab_size=...)` instead of
     `TransformerConfig.olmo3_7B(...)`. olmo2_1B_v2 is the match for
     OLMo-2-0425-1B-Instruct: its config.json is d_model(hidden_size)=2048,
     n_layers=16, n_heads=n_kv_heads=16, rope_theta=500000,
     intermediate_size=8192, max_position_embeddings=4096, vocab_size=100352,
     model_type=olmo2 — and olmo2_1B_v2 = llama2_1B(d_model=2048) with n_layers=16,
     hidden_size_multiplier=1.5 (=> FFN 8192), rope_theta=500000, qk_norm,
     reordered_norm. (The plain `olmo2_1B` factory uses llama2_1B's default
     n_layers=18, which does NOT match this 16-layer checkpoint.) The HF->distcp
     converter expects --model-arch=olmo2_1b_v2 for this (run.sh sets it).
  B. NO YaRN RoPE SCALING: olmo3_7B appends `.with_rope_scaling(YaRNRoPEScalingConfig(...))`
     for its 64k long-context. Olmo-2 1B is a native 4096-context model with plain
     RoPE (rope_theta=500000, rope_scaling=null) — so we DROP the YaRN call entirely.
     (The HF->distcp converter also asserts olmo2 arch + max_seq_len==4096, i.e. no
     RoPE extension, which is consistent.)
  C. NATIVE SEQ LEN: DEFAULT_SEQUENCE_LENGTH = 4096 (the model's native context),
     not 16384/65536. At seq 4096 no context parallelism engages (4096 <= the
     16384 tok/rank cap), which is exactly what we want for a single-GPU smoke.
  Tokenizer is the SAME dolma2 tokenizer (padded_vocab_size 100352) as the 7B
  path, so the repo's existing dolma2-tokenized .npy data is reused as-is.
...
NOTHING ELSE DEVIATES from the 7B local script — optimizer (SkipStepAdamW), hsdp, selective AC
(feed_forward), compile_model=True, generate_doc_lengths=True are all identical (the model factory /
seq-len / no-YaRN changes A–C above are the only substantive differences; the 7B keeps olmo3+YaRN).
...
# (7B imports YaRNRoPEScalingConfig here; this Olmo-2 1B uses native RoPE — see header DIFF B)
...
DEFAULT_SEQUENCE_LENGTH = 4_096  # DIFF C: Olmo-2 1B native context (vs 7B's 16_384)
DEFAULT_NUM_NODES = 1
...
def build_sft_dataset(
    ...
    dataset = NumpyPackedFSLDatasetConfig(
        ...
        # ENV-GATED (1b local test): OLMO_DOC_MASKING=0 disables intra-document masking so the SDPA/torch
        # attention backend works (it raises on cu_doc_lens). Cross-doc attention is wrong for a real model
        # but fine for a pipeline / container write-correctness test. Default ON (faithful) if unset.
        generate_doc_lengths=os.environ.get("OLMO_DOC_MASKING", "1") != "0",
...
class SFTConfig(Config):
    ...
    def build(
        ...
        *,
        ...
        # DIFF A + B: Olmo-2 1B factory (matches allenai/OLMo-2-0425-1B-Instruct's
        # config.json — d_model 2048 / 16 layers / 16 heads / rope_theta 500000 /
        # FFN 8192), and NO YaRN .with_rope_scaling() — Olmo-2 1B is native 4096-ctx
        # plain RoPE. See header DIFFs A/B. Same env-gated model_overrides as 7B.
        ...
        config = SFTConfig(
            ...
            .with_callback(
                ...
                WandBCallback(
                    ...
                    project=os.environ.get("WANDB_PROJECT", "olmo2-1b-sft"),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="SFT the Olmo-2 1B model (local pipeline test).",
