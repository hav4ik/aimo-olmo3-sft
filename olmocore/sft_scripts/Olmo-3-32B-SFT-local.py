"""
32B SFT — a VERBATIM copy of our `Olmo-3-7B-SFT-local.py` with the model swapped to the 32B
arch (`TransformerConfig.olmo3_32B`). Per spec: "match the 7B script, just the 32B model."
The ONLY differences from the 7B-local are the arch (line ~512), the W&B project default, and
labels — everything else (Beaker stubs + every env-gated knob) is IDENTICAL to the 7B-local, so
re-diff THOSE against `Olmo-3-7B-SFT-local.py` (not directly against upstream) when bumping the pin.

NOTE on AI2's upstream 32B divergence: `OLMo-core/.../Olmo-3-32B-SFT.py` HARDCODES 32B-specific
parallelism — AC `budget=0.3`, CP `llama3(degree=8, head_stride=4)`, full-FSDP `shard_degree=8`.
We deliberately do NOT hardcode those; they remain ENV-TUNABLE here exactly as in the 7B-local
(`OLMO_AC_BUDGET=0.3`, `OLMO_CP_STYLE=llama3`, etc.), so pass them at launch to reproduce AI2's 32B
shape. The YaRN rope_scaling (factor=8, 8192 base) is the same for both sizes.
