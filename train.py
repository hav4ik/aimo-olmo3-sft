...
_MAP = [
    ...
    ("optim",              "OLMO_OPTIM",               "optimizer: fused_adamw (default, fp32 state) | skip_step (spike-protected; pair with --optim-dtype bf16 for ~16GB/rank less). NOTE: adamw8bit is UNSUPPORTED under FSDP2 (bitsandbytes has no DTensor support) and errors at config time"),
    ("optim_dtype",        "OLMO_OPTIM_DTYPE",         "Adam moment dtype for --optim skip_step: bf16 stores m/v in bf16 (~16GB/rank less, fp32 master kept). Ignored by fused_adamw"),
